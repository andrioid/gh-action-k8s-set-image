...
test("if a single string is converted correctly", () => {
  ...
  const res = imageLinesToKubelines(SINGLE_LINE, "default");
  expect(res.length).toBe(1);
});

test("if a multi-line works", () => {
  const res = imageLinesToKubelines(MULTI_LINE, "default");
  expect(res.length).toBe(3);
  console.log(res);
});

test("grouping of same deployment into fewer lines", () => {
  const res = imageLinesToKubelines(MULTI_LINE_GROUP, "default");
